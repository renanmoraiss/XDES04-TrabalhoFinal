...
const AlterarLivro = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      const livro = livros.find(l => l.id === id && l.ativo !== false)
      if (livro) {
        setFormData(livro)
      }
    }
  ...
  const validarFormulario = (): boolean => {
    ...
    if (!formData.isbn || formData.isbn.trim().length === 0) {
      ...
      // Verificar duplicidade (exceto o próprio livro)
      const livros = storageService.getLivros()
      const livroExistente = livros.find(l => 
        l.ativo && 
        l.id !== id &&
        l.isbn.replace(/[-\s]/g, '') === formData.isbn?.replace(/[-\s]/g, '')
      )
      if (livroExistente) {
        novosErros.isbn = 'Já existe outro livro com este ISBN'
      }
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validarFormulario() && id) {
      ...
      const livroOriginal = storageService.getLivros().find(l => l.id === id)
      if (livroOriginal) {
        const alteracoes: Record<string, { anterior: any; novo: any }> = {}
        Object.keys(formData).forEach(key => {
          if (JSON.stringify(livroOriginal[key as keyof Livro]) !== JSON.stringify(formData[key as keyof Livro])) {
            alteracoes[key] = {
              anterior: livroOriginal[key as keyof Livro],
              novo: formData[key as keyof Livro]
            }
          }
        })
      ...
      storageService.saveLivro({ ...formData, id } as Livro)
      navigate('/livros')
    }
  }
